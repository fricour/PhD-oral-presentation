...
[Towards a]{style="font-size: 30px;"}<br> <!-- <span style="font-size: 30px;">a</span><br> --> [<strong>new insight</strong>]{style="font-size: 50px;"}<br> [of the]{style="font-size: 30px;"}<br> [<strong>carbon transport</strong>]{style="font-size: 50px;"}<br> [in the]{style="font-size: 30px;"}<br> [<strong>global ocean</strong>]{style="font-size: 50px;"}<br>
:::
...
Winter --- Low phyto growth<br> Spring --- Stratification <br><br> Bloom --- BGP trigger <br> Zoo graze and develop <br><br> [Sinking particles]{style="color: white;background-color:#001852;border-radius: 10px"}<br> [Fecal pellets (fast)<br>]{style="font-size: 24px;"} [Phyto aggregates<br>]{style="font-size: 24px;"} [By-products<br>]{style="font-size: 24px;"}
:::
...
::: {style="position:absolute;top:25%;left:72%;"}
Animal feeding<br> Microbial degradation<br> Fragmentation<br> ... <br><br> {{< fa circle-exclamation >}} Sinking speed<br> {{< fa circle-exclamation >}} Residence time<br> {{< fa circle-exclamation >}} Depth reached<br> <br> [Remineralization to CO<sub>2</sub>]{style="color: white;background-color:#001852;border-radius: 10px"}
:::
:::
...
Night --- Surface feeding<br><br> Day --- Avoid predation<br><br> Metabolize food at depth<br><br> [POC injection below EZ]{style="color: white;background-color:#001852;border-radius: 10px"}<br> (e.g. fecal pellets) <br>
:::
...
Summer --- Accumulating<br>carbon-rich lipids<br><br> Winter --- Diapause (\> MLD)<br><br> Survive on reserves<br><br> [Release CO<sub>2</sub> at depth ]{style="color: white;background-color:#001852;border-radius: 10px"}<br> Sinking of carcasses
:::
:::
...
::: {style="position:absolute;top:35%;left:72%;"}
1D mechanism<br><br> Mid and high latitudes<br><br> MLD transitions<br><br> [Isolation of POC and DOC ]{style="color: white;background-color:#001852;border-radius: 10px"}from the surface
:::
...
::: {style="position:absolute;top:35%;left:72%;"}
2D mechanism<br><br> Fronts and eddies<br><br> Strong vertical circulation<br><br> days \~ 1-10 km
:::

### Large-scale subduction

2D mechanism<br><br> Meridional overturning and Ekman<br><br> Slow advection<br><br> \~100-1000 km
:::
...
{{< fa question-circle >}} How much carbon is exported/injected by each pump? {{< fa question-circle >}} How deep is it exported/injected? {{< fa question-circle >}} How long is it sequestered from the atmosphere?
:::

::: {.fragment .fade-in-then-semi-out}
{{< fa lightbulb >}} Observational framework (Claustre et al., 2021)
:::

::: {.fragment .fade-in-then-out}
{{< fa ship >}} Research vessels {{< fa satellite >}} Satellites {{< fa anchor >}} Instrumented moorings {{< fa robot >}} Floats — gliders — USVs
:::
:::
...
::: {.fragment .fade-in-then-semi-out}
{{< fa gears >}} UVP6 and the zooplankton embedded classificationCharacterization of particles — VMP — Mesopelagic processes 
:::

::: {.fragment .fade-in-then-semi-out}
{{< fa robot >}} UVP6 and OST deployed in the Labrador SeaTwo independent measurements of particle and carbon fluxes
:::

::: {.fragment .fade-in-then-semi-out}
{{< fa book >}} Reassessment of global sequestration fluxes by the BCPContinuous approach to carbon storage ($\geq$ 100 years)
:::
:::
...
Since 2008 --- UVP5 (small enough for CTD frames)<br> 2019 --- First UVP6 deployments<br> 2022 --- First UVP6 deployments with the [embedded classification]{style="color: white;background-color:#001852;border-radius: 10px"}
:::
...
::: {.column width="50%"}
0.7L imaged volume <br><br> Particle counter (0.1 - 2.5 mm)<br> [Particle classifier]{style="color: white;background-color:#001852;border-radius: 10px"} (\> 0.6 mm)<br><br> Particle size distribution (PSD)<br> Carbon flux estimations<br> Organism studies (e.g VMP)<br>
:::
:::
:::
...
::: {style="position:absolute;top:15%;"}
::: {.fragment .fade-in-then-semi-out}
{{< fa clock >}} Image transmission time
...
::: columns
::: {.column width="55%"}
{{< fa question-circle >}} Best image classifier?
...
::: {.fragment .fade-in-then-semi-out}
{{< fa lightbulb >}} Extreme gradient boosting
...
::: {.column width="50%"}
{{< fa circle-info >}} Basic principle

::: small-text
Training set = image features<br><br> Left --- 3 predictors (decision trees)<br> Each one correcting its predecessor<br> <!-- (= fit new predictor to residual error)<br><br> --> <br>Right --- Ensemble's prediction <br><br>
:::

{{< fa battery-half >}} Energy constraints
...
{{< fa image >}} Characteristics 

[Geometry]{style="font-size: 25px;"}<br> [Gray levels]{style="font-size: 25px;"}<br> [Statistical moments]{style="font-size: 25px;"}<br><br>

{{< fa person-digging >}} Model training Training set (90%) — test set → Start training
:::
...
::: small-text
::: highlight-high
[Highest column value]{style="font-size: 25px;"}<br>
:::

::: highlight-low
[Lowest column value]{style="font-size: 25px;"}<br>
:::
:::
:::
...
{{< fa person-chalkboard >}} Acantharia (test set evaluation) TP = 51 — FP (col) = 6 — FN (row) = 23  Precision = 51/(51+6) = 89% Recall (here diagonal) = 51/(51+23) = 69%
...
Detritus --- ubiquitously messy<br> [Variety of shapes --- sizes --- gray levels --- ...<br>]{style="font-size: 20px;"}<br>

{{< fa circle-xmark >}} Undersampling {{< fa circle-xmark >}} Quantification {{< fa circle-xmark >}} Clustering (left) Continuum in the features space Inter-group confusion Does not improve biological score {{< fa circle-check >}} Add weights on biological classes
:::
:::
...
<span style="font-size: 30px;">Float 6904240 --- 21<sup>st</sup> cycle @1000 m<br><br> <span style="font-size: 30px;">New statistical jump ( --- ) detection approach<br><br> F<sub>small</sub> $\propto$ weighted average [ATN slope]{style="border-bottom: 4px solid #DCEEF3;"} <!-- F<sub>large</sub> $\propto$ sum of <span style="border-bottom: 4px solid #E31B23;">positive jumps</span> --> F<sub>large</sub> $\propto$ sum of positive jumps () <!-- F<sub>small</sub> = 633 x (ATN<sub>slope</sub>)<sup>0.77</sup> <span style="color: gray;"><br>(Estapa et al., 2023)</span><br><br> --> <!-- F<sub>large</sub> = 633 x (Jump<sub>total</sub>/drifting time)<sup>0.77</sup></span> -->
:::
...
Similar patterns across floats<br> Unclear particle signature<br><br> Rapid emptying in November<br><br> Particle increase in April
:::
...
::: {style="position:absolute;top:30%;left:65%;"}
Similar measurements across floats<br><br> Latent dynamics of the system?
:::
...
[→ malfunctioning OST since Nov 2022<br>→ float not answering since end of March]{style="font-size: 25px;"}
:::
...
POC (all) and DOC ([mixing]{style="border-bottom: 4px solid red;"})<br><br> Gravitational settling or injection<br><br> Mixing + VMP merge back with BGP
:::
...
[Continuous vs. fixed sequestration depth (e.g. 1000 m)<br><br>]{style="font-size: 30px;"} [BCP = BGP + VMP + Mixing pumps transport ]{style="font-size: 30px;"}[<strong>F<sub>org</sub></strong>]{style="color: #528133;"}<br><br> [<strong>F<sub>org</sub></strong>]{style="color: #528133;"} respired to CO<sub>2</sub> (<strong><strong>[F<sub>remin</sub></strong>]{style="color: #D01556FF;"}</strong>) or sedimented <br><br> [Respiratory CO<sub>2</sub> = biogenic DIC (<strong>]{style="font-size: 30px;"}[<strong>DIC<sub>bio</sub></strong>]{style="color: #793da5;"}</strong>)<br><br> [<strong>]{style="font-size: 30px;"}[<strong>DIC<sub>bio</sub></strong>]{style="color: #793da5;"} </strong> in the ocean for $\geq$ 100 years = sequestered (<strong><strong>[F<sub>seq</sub></strong>]{style="color: #4774c5;"}</strong>)<br><br> [Carbon flux]{style="font-size: 30px;color: white;background-color: #001852;border-radius: 10px;"}[ vs. ~~carbon inventory~~ → Pg C year<sup>-1</sup><br><br>]{style="font-size: 30px;"}
:::
...
[f<sub>100</sub> --- <em>fraction of a water parcel at a given location and depth that will remain in the ocean for $\geq$ 100 years</em><br><br>]{style="font-size: 30px;"} <span style="font-size: 30px;">OCIM model [(Siegel et al., 2021)]{style="color: gray;"}<br><br> [f<sub>100 (avg)</sub> (530 m) = 0.3<br>]{style="font-size: 30px;"} [f<sub>100 (avg)</sub> (\> 1000 m) = 0.87<br><br>]{style="font-size: 30px;"}
...
[F<sub>seq</sub> = f<sub>100</sub> x F<sub>remin</sub>]{style="font-size: 30px;color: white;background-color: #001852;border-radius: 10px"}
:::
...
[F<sub>remin</sub> = f(pixel, depth layer, C#)<br><br>]{style="font-size: 30px;"} [BGP empirical --- Martin's flux attenuation curve<br><br>]{style="font-size: 30px;"} [Mixing --- 3-in-1<br><br>]{style="font-size: 30px;"} [VMP diel --- migration at 300 m<br>]{style="font-size: 30px;"} [VMP seasonal --- migration at 600 m<br>(NNA only)]{style="font-size: 30px;"}
:::

## Examples of stations --- [F<sub>remin</sub>]{style="color: #D01556FF;"} x [f<sub>100</sub>]{style="color: #E6AB02;"} = [F<sub>seq</sub>]{style="color: #4774c5;"}

![](converse/Fig4_PHDPREZ.svg){.absolute top="40" left="-80" width="800" height="800"}

::: {style="position:absolute;top:15%;left:60%;"}
[F<sub>seq</sub>]{style="color: #4774c5;"} max above 2000 m<br><br> [50% of F<sub>seq</sub> accumulation by 1000 m<br><br>]{style="font-size: 30px;"} [Fixed sequestration depth = missing most of ]{style="font-size: 30px;"}[F<sub>seq</sub>]{style="color: #4774c5;"}<br><br>
:::
...
[Magnitudes differ but same patterns<br><br>]{style="font-size: 30px;color: white;background-color: #001852;border-radius: 10px;"}
...
[Mixing --- 11-23% of F<sub>seq</sub>(BCP)<br>]{style="font-size: 30px;"} [VMP --- 9-12% of F<sub>seq</sub>(BCP)]{style="font-size: 30px;"}
:::
