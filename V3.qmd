...
<!-- # <span style="color: white;background-color:#001852;border-radius: 10px">420 ppm and counting (1/5) </span> {background-image="intro/ppm.JPG"} -->

# <span style="color: black;">420 ppm and counting (1/5) </span> {background-image="intro/ppm.JPG" background-opacity=0.5}

## Absorbed CO<sub>2</sub> reacts with seawater

::: columns
::: {.column width="60%"}
<br>Fossil fuel emissions --- 35 Gt CO<sub>2</sub> year<sup>-1</sup></sub><br><br>
25% absorbed by land<br><br>
25% absorbed by <span style="color: white;background-color:#001852;border-radius: 10px">oceans</span><br>
...
{{< fa question-circle >}} What processes act <ins>against</ins> the vertical mixing of DIC?
:::
...
<span style="color: black;background-color:#f5e8c8;border-radius: 10px">Gravitational --- 70% export flux</span><br><br>
<span style="color: black;background-color:#e01f54;border-radius: 10px">Mixing --- 20% export flux</span><br>Sequestration from 25 to 150 years<br><br>
<span style="color: white;background-color:#001852;border-radius: 10px">Migrant --- 10% export flux</span><br>Sequestration up to 250 years (diel)<br> up to 500 years (seasonal)<br>
:::
...
<span style="color: white;background-color:#001852;border-radius: 10px">How much</span> carbon is exported/injected by each pump?<br><br>
<span style="color: white;background-color:#001852;border-radius: 10px">How deep</span> is it exported/injected?<br><br>
<span style="color: white;background-color:#001852;border-radius: 10px">How long</span> CO<sub>2</sub> is sequestered from the atmosphere?<br><br>
...
{{< fa gears >}} <span style="color: white;background-color:#001852;border-radius: 10px">UVP6</span> and a zooplankton <span style="color: white;background-color:#001852;border-radius: 10px">embedded classification</span><br><span style="color: black;background-color:white;border-radius: 10px">Zooplankton image classification model under strict energy constraints</span><br><br>
<ins>Sensor development</ins> <span style="color: gray;">&#10230; Sensor application &#10230; Global model</span> 

:::
:::

## {{< fa gears >}} UVP6 particle characterization using ML
...
::: {style="position:absolute;top:15%;opacity:0.5"}
<span style="color: white;background-color:#001852;border-radius: 10px">How much</span> carbon is exported/injected by each pump?<br><br>
<span style="color: white;background-color:#001852;border-radius: 10px">How deep</span> is it exported/injected?<br><br>
<span style="color: white;background-color:#001852;border-radius: 10px">How long</span> CO<sub>2</sub> is sequestered from the atmosphere?<br><br>
:::
...
::: {style="position:absolute;top:60%;"}
{{< fa robot >}} <span style="color: white;background-color:#001852;border-radius: 10px">UVP6</span> and <span style="color: white;background-color:#001852;border-radius: 10px">OST</span> deployed in the Labrador Sea<br><span style="color: black;background-color:white;border-radius: 10px">Two independent measurements of particle and carbon flux dynamics</span><br><br>
<span style="color: gray;">Sensor development &#10230; </span><ins>Sensor application</ins><span style="color: gray;"> &#10230; Global model</span> 
:::
...
![](images/schema-cycle-du-flotteur.png){.absolute top="27%" left="45%" width="700" height="460"}
:::
:::
...
::: {style="position:absolute;top:60%;"}
{{< fa book >}} Reassessment of <span style="color: white;background-color:#001852;border-radius: 10px">global sequestration fluxes</span> by the <span style="color: white;background-color:#001852;border-radius: 10px">BCP</span><br><span style="color: black;background-color:white;border-radius: 10px">A continuous approach to carbon storage ($\geq$ 100 years)</span><br><br>
 <span style="color: gray;">Sensor development &#10230; Sensor application &#10230; </span><ins>Global model</ins> 
:::

## {{< fa book >}} A continuous approach to carbon storage by the Biological Carbon Pump

::: footer
Left figures (2) @Thomas Boniface --- Right figure @Euro-Argo
:::
...
::: {.fragment .fade-in-then-out}
![](intro/argomap.png){.absolute top="30%" left="40%" width="600"}
:::
...
::: {style="position:absolute;top:15%;"}
How much carbon is exported/injected by each pump?<br>
How deep is it exported/injected?<br>
How long is it sequestered from the atmosphere?<br><br>
:::

::: {style="position:absolute;top:42%;"}

{{< fa gears >}} <span style="color: white;background-color:#001852;border-radius: 10px">UVP6</span> and a zooplankton <span style="color: white;background-color:#001852;border-radius: 10px">embedded classification</span><br><span style="color: gray;background-color:white;border-radius: 10px">Zooplankton image classification model under strict energy constraints</span><br>
...
# {{< fa gears >}} UVP6 embedded classification (2/5) {background-image="images/uvp6_images/mozaic_best.png" background-opacity=0.5} 
...
::: {.column width="50%"}
<span style="color: white;background-color:#001852;border-radius: 10px">Basic principle</span>
...
<span style="color: white;background-color:#001852;border-radius: 10px">Test set evaluation of labels</span><br>
<span style="font-size: 24px;"><strong>Precision</strong> = true positives over true and false positives</span><br style="line-height: 0.5em;">
<span style="font-size: 24px;"><strong>Recall</strong> = true positives over true positives and false negatives</span><br><br>
...
::: {style="position:absolute;top:25%;left:50%;"}
<br>16 biological classes<br> 
4 non-living classes<br><br>
<span style="color: white;background-color:#001852;border-radius: 10px">Global stats compared with a CNN</span><br>Accuracy --- 91% (CNN 94%)<br><br><span style="color: white;background-color:#001852;border-radius: 10px">Living weighted average</span><br>Precision --- 61% (CNN 71%)<br>Recall --- 56% (CNN 82%)
...
-   Classifying plankton is [challenging]{style="color: white;background-color: #001852;border-radius: 10px"}
    -   Adding constraints makes it even more [difficult]{style="color: white;background-color: #001852;border-radius: 10px"}
...
# {{< fa robot >}} UVP6 and OST in the Labrador Sea (3/5) {background-image="labrador/float_on_site.jpeg" background-opacity=0.5}
...
## Reminder ---  From sensor development to sensor application
...
::: small-text
Small particle flux (F<sub>small</sub>)<br>
Large particle flux (F<sub>large</sub>)
:::
:::
:::
:::
...
<span style="font-size: 30px;">New jump detection (<span class="red-dot"></span> --- <span class="yellow-dot"></span>)<br><br>
...
[Seasonal dynamics]{style="color: white;background-color:#001852;border-radius: 10px"}
:::
...
::: {style="position:absolute;top:30%;left:70%;"}
7 size classes > 0.5 mm<br><br>
Export pulse?<br><br> 1 profile every 10 days
:::
...
14 size classes (0.1 - 2.5 mm)<br><br>
...
[(A, B) from Guidi et al., (2008)]{style="color: #4774c5;"}<br> [Open ocean sites w/ low carbon fluxes]{style="color: #4774c5;"}<br>
[(A, B) from Iversen et al., (2010)<br>]{style="color: #D01556FF;"} [Highly productive system (upwelling)]{style="color: #D01556FF;"}<br>
:::
...
Yellow float --- OST malfunction since November 2022<br>
Green float --- Iceberg collision in late March?
:::
...
-   OST and UVP6 are [well correlated]{style="color: white;background-color: #001852;border-radius: 10px"}
    - Do we need both instruments on a float ? (price?)
:::
...
# {{< fa book >}} CONVERSE (4/5) {background-color="white" background-image="converse/ocean.jpg" background-opacity=0.5}
...
## Reminder --- Global approach of the BCP while waiting for more BGC-Argo floats {.smaller}
...
<span style="font-size: 30px"><em>Sequestration occurs when DIC is hidden from the atmosphere for $\geq$ 100 years</em></span><br><br>
...
<span style="font-size: 30px;color: white;background-color: #001852;border-radius: 10px">f<sub>100</sub> = 1, F<sub>remin</sub> entirely sequestered</span><br><br>
...
<span style="font-size: 30px;color: white;background-color: #001852;border-radius: 10px;">50% of F<sub>seq</sub> accumulation by 1000 m</span>
...
<span style="font-size: 30px;color: black;background-color: #f5e8c8;border-radius: 10px;">F<sub>seq</sub> (gravitational)</span>
:::

::: {style="position:absolute;top:50.5%;left:0%;"}
<span style="font-size: 30px;color: black;background-color: #e01f54;border-radius: 10px;">F<sub>seq</sub> (BCP)</span>
:::

::: {style="position:absolute;top:80.5%;left:0%;"}
<span style="font-size: 30px;color: white;background-color: #001852;border-radius: 10px;">F<sub>seq</sub> (POC, 1000 m)</span>
:::
...
::: {style="position:absolute;top:20%;left:60%;"}
<span style="font-size: 30px;color: black;background-color: white;border-radius: 10px;">7 versions (left --- 4<sup>th</sup>)<br></span>
:::

::: {style="position:absolute;top:70%;left:60%;"}
<span style="font-size: 30px;color: black;background-color: white;border-radius: 10px;">Mixing --- 11-23% of F<sub>seq</sub>(BCP)<br></span>
<span style="font-size: 30px;color: black;background-color: white;border-radius: 10px;">Migrant --- 9-12% of F<sub>seq</sub>(BCP)</span>
:::
...
## I said earlier that something was missing
...
![](intro/argomap.png){.absolute top="25%" left="40%" width="600"}
...
# Thank you ! {background-image="thanks/merci.jpg" background-opacity=0.5}

::: footer
@Me ! Maybe after my 150<sup>th</sup> Col d'Èze
:::

::: {style="font-size:30px;position:absolute;top:100%;left:0%;"}
<strong>Jury Members</strong> --- M. Estapa - S. Henson - I. Cetinić - G. Neukermans - L. Stemmann<br>
<strong>Supervisors</strong> --- Hervé - Lionel - Marilaure - Alexander<br>
<strong>COMPLEx Team</strong> --- Jean-Olivier, Louis, Marc and everyone, I've learnt a lot from you all !<br><br>
<strong>Office (and also {{< fa person-biking >}}) mates</strong> --- Thelma, Laetitia & Ophélie<br>
<strong>Friends</strong> --- From Belgium, UK, Lebanon and even some French !<br>
<strong>Family</strong> --- See you in one week ! {{< fa dragon >}}<br>
<strong>Lucie</strong> --- Especially the last few weeks !<br><br>
