...
## BCP and its export pathways

```{r, bcp and its export pathways for converse, fig.width=6, fig.height=6}

df <- tibble(group = c('BGP', 'BGP', 'VMP diel', 'VMP diel', 'VMP seasonal', 'VMP seasonal', 'Mixed layer', 'Mixed layer', 'Eddy subduction', 'Eddy subduction', 'Large-scale subduction', 'Large-scale subduction'),
...
plot_ly(df, x = ~group, y = ~start, type = 'scatter', mode = 'markers', marker = list(color = 'white', showlegend = F)) |>
  ...
  add_segments(x = 'Large-scale subduction', xend = 'Large-scale subduction', y = 200, yend = 1000, line = list(width = 15, color = "#e01f54"), showlegend = F) |>
   layout(xaxis = list(title = 'Pumps', tickfont = list(size = 20), titlefont = list(size = 20), showgrid = T), 
          ...
          tickmode = "array", tickvals = c(200,400,1000,1400), ticktext = c("200","400","1000","1400"))) |> config(displayModeBar = FALSE) |>
  add_trace(y = 2500, type = 'scatter', mode = 'line', line = list(color = 'black', dash = 'dash')) |>
  add_annotations(x = 'BGP', y = 2550, text = 'sediment')
