...
tmp <- vroom::vroom('images/uvp6_images/features_test.csv') |> select(area, mean, eccentricity, labels, objid) |> mutate(image = "") |> select(image, labels, everything()) |> mutate(mean = floor(mean), eccentricity = round(eccentricity, 2))
...
ggplot(comparison_flux_ost_uvp) + geom_line(aes(x = date, y = total_flux), colour = 'black') + facet_wrap(~wmo~park_depth, scales = 'free_y', nrow = 4) +
  geom_errorbar(aes(x = date, y = guidi_flux),
                      ...
                      fun = median, colour = '#D95F02', width = 5) +
    geom_line(aes(x = date, y = guidi_flux),
                      stat = 'summary',
                      fun = median, colour = '#D95F02') +
    geom_errorbar(aes(x = date, y = iversen_flux),
                      stat = 'summary',
                      #fun.min = function(z) {quantile(z, 0.25)},
                      #fun.max = function(z) {quantile(z, 0.75)},
                      fun = median, colour = '#E6AB02', width = 5) + # #003366
    geom_line(aes(x = date, y = iversen_flux),
                      stat = 'summary',
                      fun = median, colour = '#E6AB02') + 
...
<span style="color: #D95F02;">Guidi et al., (2008) --- (A, B) = (12.5, 3.81)</span><br>
<span style="color: #E6AB02;">Iversen et al., (2010) --- (A, B) = (273.8, 4.27)</span><br>
...
## Conclusion after 1 year of data

- OST and UVP6 are well correlated, especially at 500 and 1000 m
- Preliminary results
- etc etc etc à discuter avec Hervé et Lionel
...
![](converse/converse_fig1.svg){.absolute top=80 left=0 width="400" height="600" .my-image}
...
df <- data.frame(version = c('C1', 'C2', 'C3', 'C4', 'C5', 'C6'),
            ...
            group = c('Group1', 'Group1', 'Group1', 'Group2', 'Group2', 'Group3'),
...
## F<sub>remin</sub> x f<sub>100</sub> = F<sub>seq</sub>

![](converse/Fig4_PHDPREZ.svg){.absolute top=40 left=-60 width="800" height="800"} 
- parler de la 4 ème colonne
- dire l'example de C# utilisé
- dire qu'après on on intègre sur tous les pixels après pour avoir les estimations de flux annuels
- code couleur, il faut que le titre matche avec les courbes et que les courbes matchent avec les couleurs des slides précédentes donc rouge, blue et une autre couleur.. jaune me va bien
