...
![](intro/global_carbon_cycle.png){.absolute top=20% width="700" height="400"}

::: {style="position:absolute;top:15%;left:55%;"}
```{r DIC}
...
```
HCO<sub>3</sub><sup>-</sup> (89%) > CO<sub>3</sub><sup>2-</sup> (10.5%) > CO<sub>2</sub><sup>*</sup> (0.5%)
...
<span style="font-size: 24px;"><span style="border-bottom: 4px solid black;"><strong>How much</strong></span> organic carbon is exported --- <span style="border-bottom: 4px solid black;"><strong>How deep</strong></span> is it transported --- <span style="border-bottom: 4px solid black;"><strong>How long</strong></span> CO<sub>2</sub> is hidden from the atmosphere ?</span>
:::
...
<!-- <strong>Sinking particles</strong><br> -->
<span style="border-bottom: 4px solid black;"><strong>Sinking particles</strong><br></span>
...
<br>
<!-- Remineralization to CO<sub>2</sub> -->
<span style="border-bottom: 4px solid black;">Remineralization to CO<sub>2</sub><br></span>
...
<!-- POC injection below EZ<br> -->
POC <span style="border-bottom: 4px solid black;">injection</span> below EZ<br>
...
Release CO<sub>2</sub> at depth <br>
...
<!-- Isolation of POC and DOC from the surface -->
<span style="border-bottom: 4px solid black;">Isolation</span> of POC and DOC from the surface
:::
...
plotly::plot_ly(df, x = ~pump, y = ~end, type = 'bar', color = ~group, colors = c("#f5e8c8", "#e01f54", '#001852'), showlegend = F) |>
  plotly::layout(xaxis = list(title = 'Pumps', tickfont = list(size = 20), titlefont = list(size = 20), showgrid = F), 
          ...
          tickmode = "array", tickvals = c(400,1000,1400), ticktext = c("400","1000","1400"))) |> plotly::config(displayModeBar = FALSE) |>
...
<!-- ![](intro/glider_2.jpeg) {.absolute width="5" height="5"} -->
<!-- ::: -->
...
Zooplankton classification model under strict energy constraints
...
2022 --- First UVP6 deployment with the <span style="color: white;background-color:#001852;border-radius: 10px">embedded classification</span>
:::
...
{{< fa bullseye >}} <ins>Improving biological score</ins><br><br>
Detritus<br>
...
<span style="font-size: 30px;">Carbon flux vs. ~~carbon inventory~~ &#8594; Pg C year<sup>-1</sup><br><br></span> 
...
- Same method applied on more <span style="color: white;background-color: #001852;border-radius: 10px">regions (9)</span><br>
